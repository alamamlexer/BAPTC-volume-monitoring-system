...
class SummaryReportsController extends Controller
{
   ...
   public function index(Request $request)
    {
        ...
        $inflow_volumes = $commodities->map(function ($commodity) use ($year) {
            ...
            return [
                ...
                'monthly_volumes' => $this->getMonthlyVolumes($commodity, $year),
                'quarterly_volumes' => $this->getQuarterlyVolumes($commodity, $year),
                'monthly_short_trip_inflow_volumes' => $this->getMonthlyShortTripInflowVolumes($commodity, $year),
        ...
        $outflow_volumes = $commodities->map(function ($commodity) use ($year) {
            ...
            return [
                ...
                'outmonthly_volumes' => $this->getOutMonthlyVolumes($commodity, $year),
                'outquarterly_volumes' => $this->getOutQuarterlyVolumes($commodity, $year),
                'monthly_short_trip_outflow_volumes' => $this->getMonthlyShortTripOutflowVolumes($commodity, $year),
   ...
   if ($user->type == 0) {
    ...
    private function getMonthlyVolumes($commodity, $year)
    {
        $monthlyVolumes = [];
    ...
    private function getOutMonthlyVolumes($commodity, $year)
    {
        $monthlyVolumes = [];
