...
class ShortTripInflowAndOutflowController extends Controller
{
    ...
    public function import(Request $request)
    {
        ...
        $worksheet = $spreadsheet->getSheetByName('SHORT TRIP');
        ...
        foreach ($worksheet->getRowIterator(8) as $row) {
            ...
            $staff_name = $worksheet->getCell("H{$rowIndex}")->getValue();
            $commodity_name = $worksheet->getCell("F{$rowIndex}")->getValue();
            $volume = $worksheet->getCell("G{$rowIndex}")->getValue();
            $plate_number = $worksheet->getCell("E{$rowIndex}")->getValue();
            $vehicle_type_name = $worksheet->getCell("J{$rowIndex}")->getValue();
            $name = $worksheet->getCell("K{$rowIndex}")->getValue();
            $facilitator_name = $worksheet->getCell("L{$rowIndex}")->getValue();
            $transaction = $worksheet->getCell("D{$rowIndex}")->getValue();
            ...
            $facilitator_id = $facilitator_name 
                ...
                ? "short trip inflow" 
                : (($transaction === "OUT") ? "short trip outflow" : null);
