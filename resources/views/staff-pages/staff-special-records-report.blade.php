...
function fetchFilteredData(page = 1) {
    ...
    // Get logged-in staff ID (Assuming it's passed in the page's JS)
    const staffId = "{{ Auth::user()->id }}"; // Add the current staff's ID from Laravel Blade
    ...
    const queryParams = new URLSearchParams({
        ...
        end_date: endDate,
        staff_id: staffId, // Send the logged-in staff's ID
        ...
        if (!Array.isArray(data.data) || data.data.length === 0) {
                ...
                data.data.forEach((transaction, index) => {
                    let editButton = (userId === transaction.staff_id)
                        ? `<a href="/staff-special-record/${transaction.id}/edit" class="btn btn-outline-primary m-1"><i class="bx bxs-edit"></i> Edit</a>`
